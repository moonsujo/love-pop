...
export function ChristmasTree(props) {
  ...
  const goldenBallMaterial = new THREE.MeshStandardMaterial({ color: '#FFD700', metalness: 0.7, roughness: 0.2 })
  const redBallMaterial = new THREE.MeshStandardMaterial({ color: '#eb1d1d', metalness: 0.7, roughness: 0.2 })
  const pinkBallMaterial = new THREE.MeshStandardMaterial({ color: '#b41ec2', metalness: 0.7, roughness: 0.2 })

  // make pink appear less often
  const getRandomMaterial = () => {
    return Math.random() < 0.3 ? pinkBallMaterial : (Math.random() < 0.5 ? redBallMaterial : goldenBallMaterial)
  }
